...
import {
  Monitor, Mic, MicOff, Hand, Circle, Pencil, Trash2, ChevronLeft, Video, Eraser, MessageSquare, BarChart3, Sparkles
} from 'lucide-react';
...
function TeacherRoomContent() {
  ...
  const [liveTally, setLiveTally] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    socket.emit('join_room', { roomId: currentRoom.id, name: user.name, role: 'teacher' });

    socket.on('poll_started', (poll: any) => {
      setActivePoll(poll);
      setPollResults(null);
    });
    ...
    socket.on('poll_live_update', (data: any) => {
      setLiveTally(data);
    });
  ...
  const handleCreatePoll = (data: { question: string; options: string[]; timeoutSec: number }) => {
    if (!pollSocketRef.current || !currentRoom) return;
    pollSocketRef.current.emit('create_poll', { roomId: currentRoom.id, ...data });
  };
  ...
  const [studentAttention, setStudentAttention] = useState<Map<string, number>>(new Map());
  ...
  useDataChannel('proxima_events', (msg: any) => {
    try {
      ...
      } else if ((event as any).type === 'attention_score') {
        const data = event as any;
        if (data.studentId && typeof data.score === 'number') {
          setStudentAttention(prev => new Map(prev).set(data.studentId, data.score));
        }
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col" style={{ height: '100vh', background: '#FFFBE6', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', bottom: '12px', left: '12px', display: 'flex', gap: '8px', zIndex: 10 }}>
              ...
              <button
                ...
                style={{
                  ...SG, fontSize: '13px', fontWeight: 700,
                  height: '36px', padding: '0 14px',
                  background: chatOpen ? '#FFD600' : '#fff',
                  border: '2px solid #000', borderRadius: '4px',
                  boxShadow: '3px 3px 0px #000', cursor: 'pointer',
                  color: '#000',
                  display: 'flex', alignItems: 'center', gap: '6px',
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = '#FFD600'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = chatOpen ? '#FFD600' : '#fff'; }}
              >
              ...
              <button
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = '#6B51EF'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = '#7B61FF'; }}
              >
                <BarChart3 size={14} /> Create Poll
              </button>
              <button
                onClick={() => setGeminiOpen(p => !p)}
                style={{
                  ...SG, fontSize: '13px', fontWeight: 700,
                  height: '36px', padding: '0 14px',
                  background: geminiOpen ? '#FFD600' : '#fff',
                  border: '2px solid #000', borderRadius: '4px',
                  boxShadow: '3px 3px 0px #000', cursor: 'pointer',
                  color: '#000',
                  display: 'flex', alignItems: 'center', gap: '6px',
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.background = '#FFD600'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.background = geminiOpen ? '#FFD600' : '#fff'; }}
              >
                <Sparkles size={14} /> AI
              </button>
            </div>
          ...
          <SidebarSection title="Engagement">
            <div className="flex flex-col gap-4">
              {participants.length === 0 ? (
                ...
                participants.map((p) => {
                  const score = studentAttention.get(p.identity) ?? 100;
                  return (
                    <div key={p.identity}>
      ...
      <PollOverlay
        ...
        isTeacher={true}
        onSubmitResponse={() => {}}
        onDismiss={() => { setPollResults(null); setActivePoll(null); }}
      />
