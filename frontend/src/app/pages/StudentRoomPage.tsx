...
import { Hand, Users, ChevronLeft, Mic, MicOff, Video, MessageSquare, Sparkles } from 'lucide-react';
...
function StudentRoomContent() {
  ...
  useEffect(() => {
    ...
    socket.emit('join_room', { roomId: room.id, name: user.name, role: 'student' });
  ...
  const handlePollResponse = (optionIndex: number) => {
    if (!studentSocketRef.current || !room || !user) return;
    studentSocketRef.current.emit('submit_poll_response', {
      roomId: room.id,
    ...
    <div className="flex flex-col" style={{ height: '100vh', background: '#FFFBE6', overflow: 'hidden' }}>
      ...
      >
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: '8px',
            background: chatOpen ? '#FFD600' : '#fff', color: '#000',
            border: '2px solid #000', borderRadius: '4px',
            height: '40px', padding: '0 20px',
            ...SG, fontSize: '14px', fontWeight: 700,
            boxShadow: '4px 4px 0px #000', cursor: 'pointer',
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: '8px',
            background: geminiOpen ? '#FFD600' : '#fff', color: '#000',
            border: '2px solid #000', borderRadius: '4px',
            height: '40px', padding: '0 20px',
            ...SG, fontSize: '14px', fontWeight: 700,
            boxShadow: '4px 4px 0px #000', cursor: 'pointer',
      ...
      <PollOverlay
        ...
        liveTally={null}
        isTeacher={false}
        onSubmitResponse={handlePollResponse}
